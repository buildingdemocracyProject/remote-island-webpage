...
var ASM_CONSTS = {
  5035392: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 5035453: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 5035517: function() {return Module.webglContextAttributes.powerPreference;},  
 5035575: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 5035630: function($0) {performance.now = function() { return $0; };},  
 5035678: function($0) {performance.now = function() { return $0; };},  
 5035726: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _ListenForChatUpdates(sessionIdPtr) {
          ...
          if (!user) {
              console.error("User not authenticated!");
              Module.SendMessage("FirebaseChat", "OnChatUpdated", JSON.stringify({ Successful: false, ErrorMessage: "User is not logged in." }));
              return;
          }
          ...
          if (window.chatListeners[sessionId]) {
          ...
          const listener = chatsRef.on("value", (snapshot) => {
              if (!snapshot.exists()) {
                  ...
                  Module.SendMessage("FirebaseChat", "OnChatUpdated", JSON.stringify({ Successful: false, ErrorMessage: "No teams found." }));
                  return;
              }
  
              console.log("Chats listened and updated on the database");
              Module.SendMessage("FirebaseChat", "OnChatUpdated", JSON.stringify({ Successful: true}));
          });
  ...
  function _ListenForGlobalVotes(sessionIdPtr) {
          ...
          const listener = globalVotesRef.on("value", (snapshot) => {
              if (!snapshot.exists()) {
                  ...
                  Module.SendMessage("FirebaseVoting", "OnGlobalVoteUpdated", JSON.stringify({ Successful: true, Votes: null }));
                  return;
              }
              ...
              console.log("✅ Global votes updated:", pollList);
              Module.SendMessage("FirebaseVoting", "OnGlobalVoteUpdated", JSON.stringify({ Successful: true, Votes: pollList }));
          });
  ...
  function _ListenForTeamVotes(sessionIdPtr, teamIdPtr) {
          ...
          const listenerKey = `${sessionId}_teamVoteListener`;
          ...
          const listener = teamVotesRef.on("value", (snapshot) => {
              if (!snapshot.exists()) {
                  ...
                  Module.SendMessage("FirebaseVoting", "OnTeamVoteUpdated", JSON.stringify({ Successful: true, Votes: null }));
                  return;
              }
  ...
  function _RetrieveChat(sessionIdPtr, teamIdPtr, lastTimestampPtr = 0) {
        const sessionId = UTF8ToString(sessionIdPtr);
        const lastTimestamp = lastTimestampPtr;
        ...
        if (!user) {
            console.error("❌ User not authenticated!");
            Module.SendMessage("FirebaseChat", "OnChatRetrieved", JSON.stringify({ Successful: false, ErrorMessage: "User is not logged in." }));
            return;
        }
  
          if (!teamId)
          {
              console.warn("This is all chat " + teamId + " and that is the teamId");
              teamId = "General";
              IsTeamChat = false;
          }
        const chatRef = firebase.database().ref(`Sessions/${sessionId}/Chats/${teamId}`);
        let query = chatRef.orderByKey().limitToLast(50);
  
        if (lastTimestamp) {
            query = chatRef.orderByKey().endBefore(String(lastTimestamp)).limitToLast(50);
        }
  
        query.once("value").then(snapshot => {
            if (!snapshot.exists()) {
                console.log("❌ No chat messages found.");
                Module.SendMessage("FirebaseChat", "OnChatRetrieved", JSON.stringify({ Successful: false, ErrorMessage: "No chat found." }));
                return;
            }
  
            const messages = snapshot.val();
            const chatList = Object.entries(messages).map(([timestamp, data]) => ({
                Timestamp: timestamp,
                Sender: data.Sender,
            ...
            Module.SendMessage("FirebaseChat", "OnChatRetrieved", JSON.stringify({ Successful: true, IsTeamChat: IsTeamChat, Messages: chatList }));
  
        }).catch(error => {
            console.error("❌ Error retrieving chat messages:", error);
            Module.SendMessage("FirebaseChat", "OnChatRetrieved", JSON.stringify({ Successful: false, ErrorMessage: error.message }));
        });
    }

  function _SavePin(sessionIdPtr, latitude, longitude, descriptionPtr) {
          const sessionId = UTF8ToString(sessionIdPtr);
          const description = UTF8ToString(descriptionPtr);
          const user = firebase.auth().currentUser;
  
          if (!user) {
              console.error("User not authenticated!");
              Module.SendMessage("SpawnOnMap", "OnPinSaved", JSON.stringify({ Successful: false, ErrorMessage: "User is not logged in." }));
              return;
          }
          ...
          pinsRef.set(pinData).then(() => {
              ...
              Module.SendMessage("SpawnOnMap", "OnPinSaved", JSON.stringify({ Successful: true }));
          }).catch(error => {
              console.error("❌ Error saving pin:", error);
              Module.SendMessage("SpawnOnMap", "OnPinSaved", JSON.stringify({ Successful: false, ErrorMessage: error.message }));
          });
      }
  ...
  function _SendChatMessage(sessionIdPtr, teamIdPtr, messagePtr) {
        ...
        if (!user) {
            console.error("❌ User not authenticated!");
            Module.SendMessage("FirebaseChat", "OnChatMessageSent", JSON.stringify({ Successful: false, ErrorMessage: "User is not logged in." }));
            return;
        }
        ...
        chatRef.set(newMessage).then(() => {
            ...
            Module.SendMessage("FirebaseChat", "OnChatMessageSent", JSON.stringify({ Successful: false, ErrorMessage: error.message }));
        });
    }
  ...
  function _SubmitPoll(sessionIdPtr, jsonVotePtr, teamIdPtr) {
          ...
          if (!voteData.InitiatorPlayerID || !voteData.Topic || !voteData.AnswerOptions) {
